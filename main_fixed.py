...
# ---- FIRST: Set Tesseract path BEFORE any other imports ----
import pytesseract
...
if not os.path.exists(TESSERACT_PATH):
    ...
    print("Please install from: https://github.com/UB-Mannheim/tesseract/releases")
    sys.exit(1)
...
try:
    ...
    print(f"⚠ Warning: Could not test Tesseract: {e}")

# ---- NOW import other modules ----
import pygetwindow as gw
from screeninfo import get_monitors

# Optional: Import Presidio (not critical)
try:
    from presidio_analyzer import AnalyzerEngine
    from presidio_analyzer.nlp_engine import NlpEngineProvider
    HAS_PRESIDIO = True
except ImportError:
    print("⚠ Warning: Presidio not installed - some features will be limited")
    HAS_PRESIDIO = False
...
print("\n" + "="*60)
print("Screen Privacy Blur - API Key & Sensitive Data Detection")
print("="*60)
print("Features:")
print("  ✓ Face detection and blurring")
print("  ✓ API Key detection via OCR")
print("  ✓ Sensitive data detection (emails, phones, SSN, etc.)")
print("  ✓ Sensitive window detection (.env, secrets, config)")
...
while True:
    ...
    if blur_entire:
        ...
        if frame_idx % OCR_EVERY == 0:
            ...
            if not ocr_failed:  # Only try OCR if it hasn't failed before
                try:
                    ...
                    print(f"⚠ OCR failed on frame {frame_idx}: {type(e).__name__}")
                    print("  Continuing without OCR detection...")
                    ocr_failed = True
